...
### Approximation of $q(x_t\vertx_0)$
...
### Parameterization of $p_{\theta}(x_{t-1}\vertx_t)$
...
2. $\mu_{\theta}(x_t, t)$
    ...
    - 메모
        ...
        - model object가 $q(x_{t-1}\vertx_t, x_0)$와 $p_{\theta}(x_{t-1}\vertx_t)$의 분포를 유사하게 하는 것인데, 이는 곧 $\mu_{\theta}$와 $\tilde{\mu}$의 MSE를 최소화하는 것. 이때 $\tilde{\mu}$는 $x_0, x_t$에 대한 함수임.
...
3. To sample $x_{t-1}$~$p_{\theta}(x_{t-1}\vertx_t)$
